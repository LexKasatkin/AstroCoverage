    ...
    try:
        with fits.open(fits_path, ignore_missing_end=True) as hdul:
            ...
            if ra is not None and dec is not None and "DATE-OBS" in header:
                t_obs = Time(header["DATE-OBS"])
                altaz = SkyCoord(ra=ra*u.deg, dec=dec*u.deg).transform_to(AltAz(obstime=t_obs, location=observer))
            ...
            return {
                ...
                "exptime": header.get("EXPTIME"), "gain": header.get("GAIN"), "offset": header.get("OFFSET"),
                "ccd_temp": header.get("CCD-TEMP"), "camera": header.get("CAMERAID") or header.get("INSTRUME"),
                ...
                "latitude": observer.lat.deg, "longitude": observer.lon.deg, "elevation": observer.height.value,
                "polygon": polygon, "moon_alt": moon_info["alt"], "moon_az": moon_info["az"],
...
def generate_database(config_path, db_path):
    ...
    with ProcessPoolExecutor() as executor:
        futures = {executor.submit(process_single_fits, (f, astap_path)): f for f in fits_files}
